package com.ihrm.pay.controller;
...
import com.ihrm.common.payUtils.AlipayConfig;
import com.ihrm.common.payUtils.LeeJSONResult;
import com.ihrm.common.payUtils.OrderStatusEnum;
import com.ihrm.domain.pay.Orders;
import com.ihrm.domain.pay.Product;
import com.ihrm.pay.service.OrdersService;
import com.ihrm.pay.service.ProductService;
...
public class AlipayController {
	...
	public Result findProducts() {
		...
		Result rese=new Result(ResultCode.SUCCESS,pList);
		return rese;
	}
	/**
	 * 获取产品列表
	...
	public Result saveProducts(@RequestBody Product product) {
		productService.save(product);
		Result rese=new Result(ResultCode.SUCCESS,product);
		return rese;
	}
	/**
	 * 进入确认页面
	...
	@RequestMapping(value = "/goConfirm/{id}",method = RequestMethod.GET)
	public Result goConfirm(@PathVariable(name = "id") String productId) throws Exception {
		Product p = productService.findById(productId);
		Result rese=new Result(ResultCode.SUCCESS,p);
		return rese;
	}
	/**
	 *
	 ...
	 * @throws Exception
	 */
	public ModelAndView createOrde2r(@RequestBody Orders order) throws Exception {
	...
	public Result createOrder(@RequestBody Orders order) throws Exception {
		...
		order.setOrderStatus(OrderStatusEnum.WAIT_PAY.key);
		ordersService.save(order);
		Result rese=new Result(ResultCode.SUCCESS,order);
		return rese;
	}
	...
	public ModelAndView goPay(@RequestBody String orderId) throws Exception {
	...
	public ModelAndView alipayReturnNotice(HttpServletRequest request, HttpServletRequest response) throws Exception {
		...
		if(signVerified) {
			...
			Orders order = ordersService.findById(out_trade_no);
			Product product = productService.findById(order.getProductId());
